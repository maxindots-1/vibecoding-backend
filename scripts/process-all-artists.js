...
/**
 * Scan all artist folders
 */
function scanAllArtists() {
  const artists = [];
    ...
    if (fs.statSync(artistPath).isDirectory()) {
      try {
        ...
        if (imageFiles.length > 0) {
          ...
          console.log(`✅ Found artist: ${getArtistName(folder)} with ${imageFiles.length} images`);
        }
...
async function processAllArtists() {
  console.log('🚀 Starting full processing of all artists...\n');
  
  // Clear existing data
  await clearExistingSketches();
  
  // Scan all artists
  const artists = scanAllArtists();
  ...
  console.log(`\n📊 Found ${artists.length} artists with sketches:`);
...
processAllArtists().catch(console.error);
