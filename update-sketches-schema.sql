-- Update Sketches Schema for GPT-4 Vision and text-embedding-3-large
...
-- Add new embedding column with 3072 dimensions (text-embedding-3-large)
ALTER TABLE sketches ADD COLUMN embedding vector(3072);

-- Recreate index for new dimensions (using hnsw for high-dimensional vectors)
CREATE INDEX sketches_embedding_idx ON sketches 
USING hnsw (embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64);

-- Update match_sketches function for new dimensions
